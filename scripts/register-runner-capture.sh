#!/usr/bin/env bash
# Register a self-hosted GitHub Actions runner for the capture device.
#
# Usage:
#   GH_OWNER=yourorg GH_REPO=ThatDAMToolbox GH_PAT=TOKEN \
#     ./scripts/register-runner-capture.sh
#
# Example:
...
curl -o actions-runner.tar.gz -L https://github.com/actions/runner/releases/latest/download/actions-runner-linux-arm64-3.651.0.tar.gz
mkdir -p ~/actions-runner && tar xzf actions-runner.tar.gz -C ~/actions-runner
cd ~/actions-runner
./config.sh --url "https://github.com/${GH_OWNER}/${GH_REPO}" \
            --token "${GH_PAT}" \
            --labels "self-hosted,linux,role-capture" \
            --unattended
sudo ./svc.sh install
sudo ./svc.sh start
