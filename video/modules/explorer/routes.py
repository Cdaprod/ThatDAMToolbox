...
log.setLevel(logging.INFO)  # Change to DEBUG for development

router = APIRouter(prefix="/explorer", tags=["explorer"])

def _stamp(request: Request) -> str:
    """Return HTTP method + path string for logs."""
...
async def list_recent(request: Request,
...
async def batch_detail(request: Request,
                       batch_id: str,
                       store: StorageEngine = Depends(_store)):     # noqa: ARG001
    t0 = time.perf_counter()
    manifest = get_manifest(batch_id)
    if manifest is None:
        log.warning("%s → /explorer/batch/%s: batch not found", _stamp(request), batch_id)
...
async def set_position(request: Request,
                       sha1: str, position: int = Body(..., ge=0),
                       store: StorageEngine = Depends(_store)):
    """
    ...
    log.info("%s → /explorer/%s: reordered to position %d", _stamp(request), sha1, position)
